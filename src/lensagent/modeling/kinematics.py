# Derived in part from lenstronomy.Analysis.kinematics_api.
# See LICENSES/lenstronomy-BSD-3-Clause.txt.
...
class KinematicsAPI:
    ...
    @staticmethod
    def _positive_scalar(value):
        """Return a finite positive scalar, else ``None``."""
        try:
        ...
        if arr.size == 0:
            return None
        scalar = float(arr.flat[0])
        if not np.isfinite(scalar) or scalar <= 0:
            return None
        return scalar

    def _fallback_theta_E(self, kwargs_lens, model_kinematics_bool=None):
        """Use a positive lens ``theta_E`` when the grid estimate is invalid."""
        if model_kinematics_bool is None:
            model_kinematics_bool = [True] * len(kwargs_lens)
        candidates = []
        for i, kwargs_lens_i in enumerate(kwargs_lens):
            if i < len(model_kinematics_bool) and not model_kinematics_bool[i]:
                continue
            theta_E = self._positive_scalar(kwargs_lens_i.get("theta_E"))
            if theta_E is not None:
                candidates.append(theta_E)
        if not candidates:
            return None
        return max(candidates)

    def _resolve_theta_E(self, kwargs_lens, theta_E, model_kinematics_bool=None):
        """Return a safe Einstein-radius scale for kinematic MGE."""
        theta_E = self._positive_scalar(theta_E)
        if theta_E is not None:
            return theta_E
        fallback = self._fallback_theta_E(kwargs_lens, model_kinematics_bool)
        if fallback is not None:
            log.debug(
                "Invalid effective Einstein radius %r; falling back to lens theta_E=%s",
                theta_E,
                fallback,
            )
        return fallback
    ...
    def galkin_settings(
        ...
        theta_E = self._resolve_theta_E(
            kwargs_lens,
            theta_E,
            self._lens_model_kinematics_bool,
        )
        if theta_E is None and (self._MGE_mass or self._analytic_kinematics):
            raise ValueError(
                "Could not determine a finite Einstein radius for kinematics."
            )
    ...
    def kinematic_lens_profiles(
        ...
        if MGE_fit is True:
            ...
            if theta_E is None:
                raise ValueError(
                    "a finite positive estimate of the Einstein radius needs to be provided to "
                    "compute the MGE!"
                )
